...
public class ChatRoomMsgListPanel {
    ...
    private class MessageLoader implements BaseFetchLoadAdapter.RequestLoadMoreListener, BaseFetchLoadAdapter.RequestFetchMoreListener {
        ...
        private void loadFromLocal() {
            ...
            fetching = true;
            MsgTypeEnum[] msgTypeEnums=new MsgTypeEnum[]{MsgTypeEnum.text,MsgTypeEnum.image,MsgTypeEnum.custom};
            NIMClient.getService(ChatRoomService.class).pullMessageHistoryExType(container.account, anchor().getTime(), LOAD_MESSAGE_COUNT, QueryDirectionEnum.QUERY_OLD,msgTypeEnums)
        ...
        private void onMessageLoaded(List<ChatRoomMessage> messages) {
            List<ChatRoomMessage> delMessages=new ArrayList<>();
            int count = messages.size();
            if (messages!=null){
                for (ChatRoomMessage chatRoomMessage:messages){
                    if (chatRoomMessage.getMsgType().equals(MsgTypeEnum.notification)){
                        delMessages.add(chatRoomMessage);
                    }else if (chatRoomMessage.getMsgType().equals(MsgTypeEnum.custom) && chatRoomMessage.getAttachment()!=null){
                        CustomAttachment customAttachment=((CustomAttachment) chatRoomMessage.getAttachment());
                        if (customAttachment.getType()== CustomAttachmentType.SIGININ || customAttachment.getType()== CustomAttachmentType.SIGINOUT){
                            delMessages.add(chatRoomMessage);
                        }
                    }
                }
                messages.removeAll(delMessages);
            }
            // 逆序
